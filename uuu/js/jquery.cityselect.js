...
//});
//1.在页面当中引用该文件
// 2.我们在#city中，放置三个select，并且三个select分别设置class属性为：prov、city、dist，分别表示省、市、区（县）三个下拉框。注意如果只想实现省市二级联动，则去掉第三个dist的select即可。
// 3.调用cityselect插件非常简单，直接调用：$("#city").citySelect();4.自定义参数调用，设置默认省市区。$("#city").citySelect({url:"js/city.min.js",prov:"湖南",//省份city:"长沙",//城市dist:"岳麓区",//区县nodata:"none"//当子集无数据时，隐藏select});
